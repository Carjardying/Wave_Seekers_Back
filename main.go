...
func addUser(db *sql.DB, u *User) (int64, error) {
    ...
    if existingID != 0 {
        // is still present → return existing ID
...
func addSpot(db *sql.DB, s *Spot) (int64, error) {
    ...
    result, err := db.ExecContext(
        context.Background(),
		`INSERT INTO spot (user_id, country_id, destination, location, lat, long, peak_season_start, peak_season_end, difficulty_level, surfing_culture, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, 
